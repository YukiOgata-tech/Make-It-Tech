...
import { voidContract } from "@/lib/contracts/server";
...
export async function POST(
  ...
  try {
    await voidContract(id, { uid: session.uid, email: session.email });
    revalidatePath("/sub/admin-console/contracts");
    revalidatePath(`/sub/admin-console/contracts/${id}`);
    return Response.json({ ok: true });
  } catch (error) {
    return Response.json(
      { error: error instanceof Error ? error.message : "契約を失効できませんでした。" },
      { status: 409 }
    );
  }
}
