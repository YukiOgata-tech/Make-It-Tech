  ...
  return (
    <div className="mx-auto max-w-6xl px-3 py-8 sm:px-6 sm:py-12 lg:px-8">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        ...
        <Card className="rounded-2xl sm:rounded-3xl"><CardHeader><CardTitle className="text-base">日時・証跡</CardTitle></CardHeader><CardContent><dl className="grid gap-5 sm:grid-cols-2"><Detail label="契約発効日" value={contract.effectiveDate} /><Detail label="作成日時" value={formatDate(contract.createdAt)} /><Detail label="送信日時" value={formatDate(contract.sentAt)} /><Detail label="閲覧日時" value={formatDate(contract.viewedAt)} /><Detail label="相手方同意日時（署名操作）" value={formatDate(contract.signedAt)} /><Detail label="電子締結最終確定日時" value={formatDate(contract.completedAt)} /><Detail label="署名URL有効期限" value={formatDate(contract.tokenExpiresAt)} /><Detail label="書類取得期限" value={formatDate(contract.documentAccessExpiresAt)} /><Detail label="Audit最終Hash" value={<code className="break-all text-xs">{contract.auditLastHash}</code>} /></dl></CardContent></Card>
      </div>

      <Card className="mt-5 rounded-2xl sm:rounded-3xl"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><FileText className="size-4" />契約書類</CardTitle></CardHeader><CardContent><div className="flex flex-wrap gap-3"><Button asChild variant="outline" className="rounded-xl"><a href={`/api/admin/contracts/${contract.id}/documents/original`} target="_blank" rel="noreferrer"><ExternalLink />原本PDF</a></Button>{contract.executedDocument ? <Button asChild className="rounded-xl"><a href={`/api/admin/contracts/${contract.id}/documents/executed`} target="_blank" rel="noreferrer"><Download />締結済みPDF</a></Button> : null}{contract.certificateDocument ? <Button asChild variant="outline" className="rounded-xl"><a href={`/api/admin/contracts/${contract.id}/documents/certificate`} target="_blank" rel="noreferrer"><ShieldCheck />締結証明書</a></Button> : null}</div><p className="mt-4 text-xs text-muted-foreground">Document SHA-256</p><code className="mt-1 block break-all text-xs">{contract.document.sha256}</code>{contract.sourceTemplate ? <div className="mt-4 rounded-xl border bg-muted/30 p-3"><p className="text-xs font-medium">使用テンプレート</p><p className="mt-1 text-sm">{contract.sourceTemplate.name} {contract.sourceTemplate.version}</p><code className="mt-1 block break-all text-[10px] text-muted-foreground">Source SHA-256: {contract.sourceTemplate.sourceSha256}</code></div> : null}{contract.sourceInputRequestId ? <div className="mt-4 rounded-xl border bg-muted/30 p-3"><p className="text-xs font-medium">相手方入力からの追跡</p><Button asChild variant="link" className="h-auto px-0 text-sm"><Link href={`/sub/admin-console/contracts/input-requests/${contract.sourceInputRequestId}`}>入力依頼を開く</Link></Button><code className="block break-all text-[10px] text-muted-foreground">Input SHA-256: {contract.sourceInputSha256}</code></div> : null}</CardContent></Card>
