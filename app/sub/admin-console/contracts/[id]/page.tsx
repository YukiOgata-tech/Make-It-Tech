  ...
  return (
    <div className="mx-auto max-w-6xl px-3 py-8 sm:px-6 sm:py-12 lg:px-8">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        ...
        <Card className="rounded-2xl sm:rounded-3xl"><CardHeader><CardTitle className="text-base">日時・証跡</CardTitle></CardHeader><CardContent><dl className="grid gap-5 sm:grid-cols-2"><Detail label="契約書記載日" value={contract.effectiveDate} /><Detail label="作成日時" value={formatDate(contract.createdAt)} /><Detail label="送信日時" value={formatDate(contract.sentAt)} /><Detail label="閲覧日時" value={formatDate(contract.viewedAt)} /><Detail label="締結日時" value={formatDate(contract.signedAt)} /><Detail label="署名URL有効期限" value={formatDate(contract.tokenExpiresAt)} /><Detail label="書類取得期限" value={formatDate(contract.documentAccessExpiresAt)} /><Detail label="Audit最終Hash" value={<code className="break-all text-xs">{contract.auditLastHash}</code>} /></dl></CardContent></Card>
      </div>
      ...
      <Card className="mt-5 rounded-2xl sm:rounded-3xl"><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle className="text-base">Audit Log</CardTitle><span className={auditVerification.valid ? "text-xs font-medium text-emerald-600" : "text-xs font-medium text-destructive"}>{auditVerification.valid ? "ハッシュチェーン検証済み" : `整合性エラー（${auditVerification.brokenAtSequence ?? "末尾"}）`}</span></div></CardHeader><CardContent><ol className="space-y-3">{events.map((event) => <li key={event.eventId} className="rounded-xl border p-3"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="font-mono text-xs">{String(event.sequence).padStart(3, "0")} {event.eventType}</strong><time className="text-xs text-muted-foreground">{formatDate(FirebaseTimestampFromIso(event.occurredAt))}</time></div><div className="mt-2 grid gap-1 text-[11px] text-muted-foreground sm:grid-cols-2"><span>actor: {event.actorType}</span><span>IP: {event.ipAddress || "-"}</span><span className="break-all">requestId: {event.requestId || "-"}</span><span className="truncate" title={event.userAgent}>UA: {event.userAgent || "-"}</span></div><details className="mt-2 text-xs"><summary className="cursor-pointer text-muted-foreground">Hash・metadata</summary><p className="mt-2 break-all font-mono text-[10px] text-muted-foreground">previous: {event.previousHash || "(genesis)"}</p><p className="mt-1 break-all font-mono text-[10px] text-muted-foreground">event: {event.eventHash}</p><pre className="mt-2 overflow-auto rounded-lg bg-muted/50 p-2 text-[10px]">{JSON.stringify(event.metadata, null, 2)}</pre></details></li>)}</ol></CardContent></Card>
    </div>
  );
}
