  ...
  return (
    <div className="mx-auto max-w-6xl px-3 py-8 sm:px-6 sm:py-12 lg:px-8">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        ...
        <Card className="rounded-2xl sm:rounded-3xl"><CardHeader><CardTitle className="text-base">日時・証跡</CardTitle></CardHeader><CardContent><dl className="grid gap-5 sm:grid-cols-2"><Detail label="契約書記載日" value={contract.effectiveDate} /><Detail label="作成日時" value={formatDate(contract.createdAt)} /><Detail label="送信日時" value={formatDate(contract.sentAt)} /><Detail label="閲覧日時" value={formatDate(contract.viewedAt)} /><Detail label="締結日時" value={formatDate(contract.signedAt)} /><Detail label="URL有効期限" value={formatDate(contract.tokenExpiresAt)} /><Detail label="Audit最終Hash" value={<code className="break-all text-xs">{contract.auditLastHash}</code>} /></dl></CardContent></Card>
      </div>
