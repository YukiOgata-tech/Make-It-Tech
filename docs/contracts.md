...
- 相手方入力画面: `sign.make-it-tech.com/i/{token}`
- ローカル管理画面: `/sub/admin-console/contracts`
- ローカル署名画面: `/sub/sign/c/{token}`
- ローカル相手方入力画面: `/sub/sign/i/{token}`
...
新規契約画面では、次の2つから作成方法を選択します。

### Make It Techが入力

管理者がテンプレート、取引先情報、署名予定者、契約条件を入力し、Wordから変換したPDFを確認して原本登録します。既存の作成フローです。
...
入力URL、署名URL、書類取得URLのトークンSHA-256をドキュメントIDとして利用します。平文トークンは保存しません。入力依頼IDまたは契約ID、用途（`recipient_input` / `signing` / `documents`）、状態、作成日時、有効期限、使用・失効日時を保持します。旧データで用途がないトークンは署名用として扱います。`recipient_input`トークンで契約PDFを取得することはできません。
...
- URLはbase64url形式
- DBにはSHA-256のみ保存
- 入力URLの既定期限は48時間。再発行すると旧入力URLを失効させ、新しい48時間のURLを発行
- 署名URLの既定期限は48時間（2日）、管理画面で1～30日を指定可能
...
登録済み署名者氏名の再入力と、以下の4項目をすべて必須とします。氏名はNFKC正規化と空白除去後に登録値と照合し、確認した氏名と文言バージョンをAudit Logへ保存します。
