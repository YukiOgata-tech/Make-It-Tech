...
管理画面の新規契約ページでは、認証済み管理者が登録済みテンプレートを選び、そのテンプレート固有の空欄へ入力してWordを生成します。V1では`nda-standard-v1`、`data-handling-addendum-standard-v1`、`fde-master-standard-v1`を選択できます。空のWordは配布しません。テンプレートは公開ディレクトリへ置かず、Route HandlerがファイルのSHA-256を検証してから読み込みます。
...
`data-handling-addendum-standard-v1`では、甲の法人名、所在地、代表者の役職・氏名、契約締結日を必須入力としてWordへ反映します。別紙の次のデータ条件は任意入力です。
...
入力した項目だけを別紙の対応欄へ反映し、未入力項目はテンプレートの標準案内文を維持します。末尾の紙契約用締結文言と押印欄は、管理画面での確認を必須にした上で電子締結用へ置き換えます。生成後は管理者がWord上で本文と別紙を最終確認し、PDFへ書き出して原本登録します。

## FDE業務委託基本契約 Wordテンプレート

`fde-master-standard-v1`では、甲の法人名、所在地、代表者の役職・氏名、契約締結日に加え、原本で空欄になっている次の契約条件を必須入力としてWordへ反映します。
