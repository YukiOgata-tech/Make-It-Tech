...
該当しない項目も「なし」「対象外」「特段の定めなし」など、契約条件として意味のある値を入力します。別紙の編集用見出し・案内文と全項目の記入例は生成時に除去され、確定値へ置き換わります。末尾の紙契約用締結文言と押印欄は、管理画面での確認を必須にした上で電子締結用へ置き換えます。生成後は管理者がWord上で本文と別紙を最終確認し、PDFへ書き出して原本登録します。
